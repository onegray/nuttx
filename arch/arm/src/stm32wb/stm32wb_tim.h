...
struct stm32wb_tim_ops_s
{
  ...
  void (*enable)(FAR struct stm32wb_tim_dev_s *dev);
  void (*disable)(FAR struct stm32wb_tim_dev_s *dev);
  int  (*setmode)(FAR struct stm32wb_tim_dev_s *dev,
                  enum stm32wb_tim_mode_e mode);
  int  (*setfreq)(FAR struct stm32wb_tim_dev_s *dev, uint32_t freq);
  int  (*setclock)(FAR struct stm32wb_tim_dev_s *dev, uint32_t freq);
  uint32_t (*getclock)(FAR struct stm32wb_tim_dev_s *dev);
  void (*setperiod)(FAR struct stm32wb_tim_dev_s *dev, uint32_t period);
  uint32_t (*getperiod)(FAR struct stm32wb_tim_dev_s *dev);
  uint32_t (*getcounter)(FAR struct stm32wb_tim_dev_s *dev);

  /* General and Advanced Timers Adds */

  int  (*setchannel)(FAR struct stm32wb_tim_dev_s *dev, uint8_t channel,
                     enum stm32wb_tim_channel_e mode);
  int  (*setcompare)(FAR struct stm32wb_tim_dev_s *dev, uint8_t channel,
                     uint32_t compare);
  int  (*getcapture)(FAR struct stm32wb_tim_dev_s *dev, uint8_t channel);

  /* Timer interrupts */

  int  (*setisr)(FAR struct stm32wb_tim_dev_s *dev,
                 xcpt_t handler, void *arg, int source);
  void (*enableint)(FAR struct stm32wb_tim_dev_s *dev, int source);
  void (*disableint)(FAR struct stm32wb_tim_dev_s *dev, int source);
  void (*ackint)(FAR struct stm32wb_tim_dev_s *dev, int source);
  int  (*checkint)(FAR struct stm32wb_tim_dev_s *dev, int source);

  /* Debug */

  void (*dump_regs)(FAR struct stm32wb_tim_dev_s *dev);
};
...
#ifdef CONFIG_TIMER
int stm32wb_timer_initialize(FAR const char *devpath, int timer);
#endif
