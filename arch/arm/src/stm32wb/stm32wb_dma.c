...
typedef FAR const struct stm32wb_dmamux_s *DMA_MUX;
...
typedef FAR struct stm32wb_dmach_s *DMA_CHANNEL;
...
struct stm32wb_dma_ops_s
{
  ...
  CODE void (*dma_sample)(DMA_HANDLE handle, struct stm32wb_dmaregs_s *regs);
...
static int stm32wb_dma12_interrupt(int irq, void *context, FAR void *arg);
...
static DMA_CHANNEL stm32wb_dma_channel_get(uint8_t channel,
                                           uint8_t controller);
static void stm32wb_gdma_limits_get(uint8_t controller, FAR uint8_t *first,
                                    FAR uint8_t *last);
