...
static inline void stm32wb_fpuconfig(void);
#endif
...
void __start(void)
{
  ...
  /* The SRAM2a region is parity checked, but upon power up, it will be in
   ...
   * if the memory is being used in standby mode.  In that case, the
   * first-time initialization needs to be performed by the board under
   * application-specific circumstances.  On the other hand, if we're using
   * this memory for, say, additional heap space, then this is handy.
   */
