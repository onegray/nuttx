...
static void stm32wb_serial_setformat(FAR struct uart_dev_s *dev);
#endif
static int  stm32wb_serial_setup(FAR struct uart_dev_s *dev);
static void stm32wb_serial_shutdown(FAR struct uart_dev_s *dev);
static int  stm32wb_serial_attach(FAR struct uart_dev_s *dev);
static void stm32wb_serial_detach(FAR struct uart_dev_s *dev);
static int  up_interrupt(int irq, FAR void *context, FAR void *arg);
static int  stm32wb_serial_ioctl(FAR struct file *filep, int cmd,
                                unsigned long arg);
#ifndef SERIAL_HAVE_ONLY_DMA
static int  stm32wb_serial_receive(FAR struct uart_dev_s *dev,
                                  FAR unsigned int *status);
static void stm32wb_serial_rxint(FAR struct uart_dev_s *dev, bool enable);
static bool stm32wb_serial_rxavailable(FAR struct uart_dev_s *dev);
#endif
#ifdef CONFIG_SERIAL_IFLOWCONTROL
static bool stm32wb_serial_rxflowcontrol(FAR struct uart_dev_s *dev,
                                        unsigned int nbuffered, bool upper);
#endif
static void stm32wb_serial_send(FAR struct uart_dev_s *dev, int ch);
static void stm32wb_serial_txint(FAR struct uart_dev_s *dev, bool enable);
static bool stm32wb_serial_txready(FAR struct uart_dev_s *dev);

#ifdef SERIAL_HAVE_RXDMA
static int  stm32wb_serial_dmasetup(FAR struct uart_dev_s *dev);
static void stm32wb_serial_dmashutdown(FAR struct uart_dev_s *dev);
static int  stm32wb_serial_dmareceive(FAR struct uart_dev_s *dev,
                                     FAR unsigned int *status);
...
#endif
static void stm32wb_serial_dmarxint(FAR struct uart_dev_s *dev, bool enable);
...
static void stm32wb_serial_pmnotify(FAR struct pm_callback_s *cb, int domain,
                                   enum pm_state_e pmstate);
static int  stm32wb_serial_pmprepare(FAR struct pm_callback_s *cb, int domain,
                                    enum pm_state_e pmstate);
#endif
...
FAR static struct stm32wb_serial_s *
const g_uart_devs[STM32WB_NUSART + STM32WB_NUART] =
{
#ifdef CONFIG_STM32WB_USART1_SERIALDRIVER
  [0] = &g_usart1priv,
#endif
...
static inline
uint32_t stm32wb_serial_getreg(FAR struct stm32wb_serial_s *priv, int offset)
{
...
static inline void stm32wb_serial_putreg(FAR struct stm32wb_serial_s *priv,
...
static inline
void stm32wb_serial_setusartint(FAR struct stm32wb_serial_s *priv,
                               uint16_t ie)
{
  uint32_t cr;
...
static void stm32wb_serial_restoreusartint(FAR struct stm32wb_serial_s *priv,
                                          uint16_t ie)
{
  irqstate_t flags;
...
static void stm32wb_serial_disableusartint(FAR struct stm32wb_serial_s *priv,
                                          FAR uint16_t *ie)
{
  irqstate_t flags;
...
#ifdef SERIAL_HAVE_RXDMA
static int stm32wb_serial_dmanextrx(FAR struct stm32wb_serial_s *priv)
{
  size_t dmaresidual;
...
static void stm32wb_serial_setformat(FAR struct uart_dev_s *dev)
{
  FAR struct stm32wb_serial_s *priv =
      (FAR struct stm32wb_serial_s *)dev->priv;
  uint32_t regval;
...
static void stm32wb_serial_setsuspend(struct uart_dev_s *dev, bool suspend)
{
  FAR struct stm32wb_serial_s *priv = (struct stm32wb_serial_s *)dev->priv;
#ifdef SERIAL_HAVE_RXDMA
...
static void stm32wb_serial_pm_setsuspend(bool suspend)
{
  ...
  for (n = 0; n < STM32WB_NUSART + STM32WB_NUART; n++)
    {
...
static void stm32wb_serial_setapbclock(FAR struct uart_dev_s *dev, bool on)
{
  FAR struct stm32wb_serial_s *priv =
      (FAR struct stm32wb_serial_s *)dev->priv;
  uint32_t rcc_en;
...
static int up_interrupt(int irq, FAR void *context, FAR void *arg)
{
  FAR struct stm32wb_serial_s *priv = (FAR struct stm32wb_serial_s *)arg;
  int  passes;
  bool handled;
                               ...
                               unsigned long arg)
{
  ...
  switch (cmd)
    ...
    case TIOCSERGSTRUCT:
      {
        FAR struct stm32wb_serial_s *user =
            (FAR struct stm32wb_serial_s *)arg;
        if (!user)
          {
            ret = -EINVAL;
          }
...
static void stm32wb_serial_dmareenable(FAR struct stm32wb_serial_s *priv)
{
#ifdef CONFIG_SERIAL_IFLOWCONTROL
  if (priv->iflow)
    {
...
#ifdef SERIAL_HAVE_RXDMA
static bool stm32wb_serial_dmarxavailable(FAR struct uart_dev_s *dev)
{
  FAR struct stm32wb_serial_s *priv =
      (FAR struct stm32wb_serial_s *)dev->priv;
...
void arm_earlyserialinit(void)
{
  ...
  for (i = 0; i < STM32WB_NUSART + STM32WB_NUART; i++)
    {
      if (g_uart_devs[i])
        {
