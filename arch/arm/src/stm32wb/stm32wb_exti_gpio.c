...
static int stm32wb_exti0_isr(int irq, void *context, FAR void *arg)
{
  int ret = OK;
...
static int stm32wb_exti1_isr(int irq, void *context, FAR void *arg)
{
  int ret = OK;
...
static int stm32wb_exti2_isr(int irq, void *context, FAR void *arg)
{
  int ret = OK;
...
static int stm32wb_exti3_isr(int irq, void *context, FAR void *arg)
{
  int ret = OK;
...
static int stm32wb_exti4_isr(int irq, void *context, FAR void *arg)
{
  int ret = OK;
...
static int stm32wb_exti_multiisr(int irq, void *context, void *arg, int first, int last)
{
  uint32_t pr;
  int pin;
  int ret = OK;
  ...
  for (pin = first; pin <= last; pin++)
    {
      ...
      uint32_t mask = EXTI_GPIO_LINE(pin);
      if ((pr & mask) != 0)
        {
          /* Clear the pending interrupt */
...
static int stm32wb_exti1510_isr(int irq, void *context, FAR void *arg)
{
                         ...
                         bool event, xcpt_t func, void *arg)
{
  ...
  uint32_t exti = EXTI_GPIO_LINE(pin);
  int      irq;
  xcpt_t   handler;
  int      nshared;
  int      i;
  ...
  modifyreg32(STM32WB_EXTI_EMR1,
              event ? 0 : exti,
              event ? exti : 0);
  modifyreg32(STM32WB_EXTI_IMR1,
              func ? 0 : exti,
              func ? exti : 0);
