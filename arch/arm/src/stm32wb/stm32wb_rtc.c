...
#ifdef CONFIG_RTC_ALARM
static int stm32wb_rtc_alarm_handler(int irq, FAR void *context,
...
int stm32wb_rtc_getdatetime_with_subseconds(FAR struct tm *tp,
                                            FAR long *nsec)
{
...
#ifdef CONFIG_RTC_PERIODIC
static int stm32wb_rtc_wakeup_handler(int irq, FAR void *context,
                                      FAR void *arg)
{
  uint32_t regval = 0;
...
#ifdef CONFIG_RTC_PERIODIC
int stm32wb_rtc_setperiodic(FAR const struct timespec *period,
                            wakeupcb_t callback)
{
